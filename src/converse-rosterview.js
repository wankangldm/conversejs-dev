...
converse.plugins.add('converse-', {
    ...
    initialize () {
        ...
        _converse.api.promises.add('Initialized');
        ...
        _converse.AddContactModal = _converse.BootstrapModal.extend({
            ...
            initXHRAutoComplete () {
                ...
                xhr.onload = () => {
                    if (xhr.responseText) {
                        ...
                        var rArry =JSON.parse(r);
                        this.name_auto_complete._list = [];
                        if (rArry.length ==undefined){
                            var data = {};
                            data["label"] = rArry.fullname || rArry.jid;
                            data["value"] = rArry.jid;
                            this.name_auto_complete._list.push(data);
                        }
                        for (var i = 0; i < rArry.length; i++) {
                            var data = {};
                            data["label"] = rArry[i].fullname || rArry[i].jid;
                            data["value"] = rArry[i].jid;
                            this.name_auto_complete._list.push(data);
                        }
                        /*this.name_auto_complete.list = rArry.map(i => ({
                          'label': i.fullname || i.jid,
                          'value': i.jid
                        }));*/
                        this.name_auto_complete.auto_completing = true;
                        this.name_auto_complete.evaluate();
                        const searching_el = this.el.querySelector('span[class="searching"]');
                        searching_el.setAttribute("hidden",true);
                    }
                };
                ...
                input_el.addEventListener('input', _.debounce(() => {
                    //过滤输入字符串的格式
                    if (input_el.value==""||input_el.value.length==0||input_el.value.match(/^\s*$/)) {
                        return;
                    }
                    searching_el.removeAttribute("hidden");
                    xhr.open("GET", "".concat(_converse.xhr_user_search_url, "q=").concat(input_el.value), true);
            ...
            initXHRFetch() {
                ...
                input_el.addEventListener('input', _.debounce(() => {
                    ...
                    this.el.querySelector('input[name="jid"]').value = "";
                    searching_el.removeAttribute("hidden");
                    xhr.open("GET", "".concat(_converse.xhr_user_search_url, "q=").concat(decodeURIComponent(input_el.value)), true);
                    xhr.setRequestHeader('Content-Type','text/html;charset=UTF-8');
                    xhr.send();
                }, 300));
                this.name_auto_complete.on('suggestion-box-selectcomplete', ev => {
                    this.el.querySelector('input[name="name"]').value = ev.text.label;
                    this.el.querySelector('input[name="jid"]').value = ev.text.value + "@" + _converse.domain;
                });
            ...
            validateSubmission (jid) {
                ...
                if (!jid || _.compact(jid.split('@')).length < 2) {
                    ...
                    el.textContent = __('您不能添加自己为联系人')
                    u.addClass('d-block', el);
                    return false;
                } else if (_converse.roster.get(Strophe.getBareJidFromJid(jid))) {
                    el.textContent = __('此联系人已经是您的好友')
                    u.addClass('d-block', el);
                    return false;
                }
            ...
            addContactFromForm(ev) {
                const name_input_el = this.el.querySelector('input[name="name"]');
                const jid_input_el = this.el.querySelector('input[name="jid"]');
                ev.preventDefault();
                const data = new FormData(ev.target),
                    jid = jid_input_el.value,
                    name = name_input_el.value;
                if (!jid) {
                    const el = this.el.querySelector('.invalid-feedback');
                    el.textContent = __('请输入用户名以查询用户,选择您要添加的联系人');
                    u.addClass('d-block', el);
                    return;
                }
                /*if (!jid && _converse.xhr_user_search_url && _.isString(_converse.xhr_user_search_url)) {
                  const input_el = this.el.querySelector('input[name="name"]');
                  this.xhr.open("GET", "".concat(_converse.xhr_user_search_url, "q=").concat(input_el.value), true);
                  this.xhr.send();
                  return;
                }*/
