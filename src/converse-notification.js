...
converse.plugins.add('converse-notification', {
    ...
    initialize () {
        ...
        _converse.api.settings.update({
            ...
            notification_icon: 'css/images/logo-大.png',
        ...
        _converse.showMessageNotification = function (message) {
            ...
            if (!_converse.areDesktopNotificationsEnabled()) {
                console.log("Notification.permission通知权限",Notification.permission);
                return;
            }
            ...
            if (message.getAttribute('type') === 'headline') {
                ...
                title = __("%1$s 发来消息:", Strophe.getResourceFromJid(full_from_jid));
            } else {
                ...
                if (!_.isUndefined(roster_item)) {
                    title = __("%1$s 发来消息:", roster_item.getDisplayName());
                } else {
                    if (_converse.allow_non_roster_messaging) {
                        title = __("%1$s 发来消息:", from_jid);
                    } else {
                        return;
                    }
                }
        ...
        _converse.handleMessageNotification = function (data) {
             ...
             */
            console.log("开始处理是否显示通知..................");
            const message = data.stanza;
            if (!_converse.shouldNotifyOfMessage(message)) {
                console.log("是否显示通知:"+_converse.shouldNotifyOfMessage(message));
                return false;
            }
            /**
